...
```
在HTTP规范中，`GET`、`POST`、`PUT`、`DELETE`分别对应`查`，`改`，`增`，`删`，这里router的方法也一一对应。通常我们使用`GET`来查询和获取数据，使用`POST`来更新资源。`PUT`和`DELETE`使用比较少，但是如果你们团队比较注重规范和语义化，就比较推荐使用了。我们注意到，上述代码中还有一个`all`方法。`all`方法用于处理上述方法无法匹配的情况，或者你不确定客户端发送的请求方法类型。
...
```
最主要的区别就是ajax中method的值，method的值就和router的方法一一对应。上述代码中没有处理异常，当请求都无法匹配的时候，我们应该跳转到404页面，比如：
```
...
```
`*`号是一种通配符，表示匹配任意URL。这里的返回是一种简化的写法，真实开发中，我们肯定要去读取html文件或者其他模板文件的内容，再响应请求。后面的章节中会详细介绍。
...
到现在为止，修改之后的代码，比起原来已经有一些优化了，但是还不够。我们可以把路由匹配单独抽象成一个`router.js`文件，把所有路由的映射关系都写在这个文件中。然后再建一个`controller`文件夹，把所有的处理函数都写在`controller`文件夹中的js文件里。在后面的章节中我们会详细介绍。
